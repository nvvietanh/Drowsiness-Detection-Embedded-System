...
//       try {
//         const response = await axios.get(`${process.env.REACT_APP_API_URL}/driver_locations/vehicle/${vehicle.vehicle_id}`);
...
//           setCoords([newCoords[newCoords.length - 1].latitude, newCoords[newCoords.length - 1].longitude]);
//         }
//       } catch (error) {
...
import React, { useEffect, useRef, useState } from "react";
import { useLocation, useParams } from "react-router-dom";
import { Box, Grid, Typography, Paper, TableContainer, Table, TableHead, TableRow, TableCell, TableBody } from "@mui/material";
...
const LocationWithLeaflet = () => {
  ...
  useEffect(() => {
    const fetchCoords = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/driver_locations/vehicle/${vehicle?.vehicle_id || vehicleId}`);
        const newCoords = response.data;
  ...
  useEffect(() => {
    // Khắc phục lỗi icon bị mất khi tải lại trang
    const customIcon = new L.Icon({
      iconUrl: "https://upload.wikimedia.org/wikipedia/commons/f/f8/PinS.png",
      iconSize: [32, 32],
      iconAnchor: [16, 32],
      popupAnchor: [0, -32],
    });
  ...
  return (
    <Box p={2} sx={{ width: "100%", height: "50vh" }}>
      <Typography variant="h6">
        Biển số: {vehicle?.license_plate || "N/A"}
      </Typography>
            ...
            >
              ...
              {/* Hiển thị các tọa độ cũ bằng CircleMarker */}
                            {path.map((coord, index) => (
                              ...
                              >
                                <Popup >
                                  Vĩ độ: {coord.latitude}, Kinh độ: {coord.longitude}
                                </Popup>
                              </CircleMarker>
                            ))}
              {/* <Polyline positions={path.map(coord => [coord.latitude, coord.longitude])} color="blue" /> */}
            </MapContainer>
          </Paper>
        </Grid>
      </Grid>
