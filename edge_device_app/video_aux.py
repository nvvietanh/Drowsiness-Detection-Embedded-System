...
def add_attendance(driver_id, vehicle_id, date):
    ...
    response = requests.post(f"http://{SERVER_URL}/attendances/add", json=attendance_data)
...
def update_attendance(attendance_id, driver_id, vehicle_id, checkin_time, checkout_time, note):
    ...
    response = requests.post(f"http://{SERVER_URL}/attendances/update", json=update_data)
...
def save_infor():
    ...
    if DRIVER_ID is None:
        ...
        ATTENDANCE_API_URL = f"http://{SERVER_URL}/attendances/search"  # Địa chỉ API điểm danh
        ...
        url = f'http://{SERVER_URL}/driver_locations/add'
        try:
